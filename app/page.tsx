...
export default function PageSelectionModule() {
  ...
  const { data: profiles } = useQuery<{ agence_nom_agence: string | null }>(
    `SELECT a.nom_agence AS agence_nom_agence
     FROM profiles p
  ...
  useEffect(() => {
    const agencyName = profiles?.[0]?.agence_nom_agence
  ...
  useEffect(() => {
    ...
    // 2. Identification de l'utilisateur; le profil est lu par PowerSync.
