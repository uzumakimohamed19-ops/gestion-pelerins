...
export default function DetailsPelerin() {
  ...
  const { data: queriedPelerin, isLoading: pelerinLoading } = useQuery<any>(
    'SELECT p.*, a.nom_agence AS agence_nom_agence FROM pelerins p LEFT JOIN agences a ON p.agence_id = a.id WHERE p.id = ?',
    [id ?? ''],
  )
  ...
  const saveAdvancedData = async () => {
    ...
    try {
      ...
      await db.execute(
        `UPDATE pelerins SET reference = ?, agence_ou_personne_associee = ?, vacciné = ?, visite_medicale = ?, formation_suivie = ?, date_formation = ?, groupe_formation = ?, hotel_mecque = ?, hotel_medine = ?, hotel_statut = ?, groupe_encadrement = ?, date_depart = ?, date_retour = ?, visa_obtenu = ?, document_url = ? WHERE id = ?`,
        [p.reference || null, p.agence_ou_personne_associee || null, p.vacciné ? 1 : 0, p.visite_medicale ? 1 : 0, p.formation_suivie ? 1 : 0, p.date_formation || null, p.groupe_formation || null, p.hotel_mecque || null, p.hotel_medine || null, p.hotel_statut ? 1 : 0, p.groupe_encadrement || null, p.date_depart || null, p.date_retour || null, p.visa_obtenu ? 1 : 0, nextDocumentUrl, p.id],
      )
      {
  ...
  useEffect(() => {
    async function getPelerin() {
      ...
      if (queriedPelerin?.[0]) {
        ...
        setPelerin({ ...current, agences: current.agence_nom_agence ? { nom_agence: current.agence_nom_agence } : undefined })
      }
        ...
        <div className="bg-white p-6 md:p-8 rounded-[2rem] shadow-xl shadow-slate-200/60 border border-slate-100/80 relative overflow-hidden transition-transform duration-300 hover:shadow-2xl">
          <div className="flex flex-col xl:flex-row justify-between gap-6 relative z-10">
            ...
            <div className="grid grid-cols-2 md:flex gap-3 items-end flex-wrap">
              ...
              <Link href={`/hajj/modifier-pelerin?id=${p.id}`} className="bg-slate-50 border border-slate-200 text-slate-700 p-4 rounded-2xl font-bold text-xs flex items-center justify-center gap-2 hover:bg-slate-100 hover:scale-[1.02] transition-all duration-200">
