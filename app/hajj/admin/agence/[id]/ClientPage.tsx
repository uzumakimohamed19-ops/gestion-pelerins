...
export default function ListeAdminPelerins() {
  ...
  const { data: queriedPelerins, isLoading: loading } = useQuery<any>(
    `SELECT * FROM pelerins ${id ? 'WHERE agence_id = ?' : ''} ORDER BY nom_complet ASC`,
    id ? [id] : [],
  )
  const pelerins = queriedPelerins ?? []
                ...
                <div key={p.id} className="bg-white border border-gray-100 rounded-[2rem] p-4 shadow-md flex flex-col justify-between relative overflow-hidden group">
                  <div>
                    ...
                    <div className="flex items-center justify-between gap-2 mb-3">
                      ...
                      <Link 
                        href={`/hajj/admin/pelerin?id=${p.id}`}
