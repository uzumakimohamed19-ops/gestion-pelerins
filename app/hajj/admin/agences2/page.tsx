...
export default function AdminAgencesList() {
  ...
  const { data: agencesData, isLoading: agencesLoading } = useQuery<AgenceStat>(
    'SELECT * FROM agences ORDER BY nom_agence ASC',
  )
  const { data: pelerinsData, isLoading: pelerinsLoading } = useQuery<any>(
    'SELECT agence_id, sur_plateforme_gouv FROM pelerins',
  )
  const loading = agencesLoading || pelerinsLoading
  const agences = useMemo<AgenceStat[]>(() => {
    const statsMap: Record<string, { total: number; gouv: number }> = {}
    for (const pelerin of pelerinsData ?? []) {
      if (!pelerin.agence_id) continue
      statsMap[pelerin.agence_id] ??= { total: 0, gouv: 0 }
      statsMap[pelerin.agence_id].total += 1
      if (Boolean(pelerin.sur_plateforme_gouv)) statsMap[pelerin.agence_id].gouv += 1
    }
    return (agencesData ?? []).map((agence: any) => ({
      ...agence,
      total_pelerins: statsMap[agence.id]?.total || 0,
      pelerins_gouv: statsMap[agence.id]?.gouv || 0,
    }))
  }, [agencesData, pelerinsData])
              ...
              >
                ...
                <div className="p-5 space-y-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <Link 
                          href={`/hajj/admin/agences2/detail?id=${agence.id}`}
                ...
                <div className="bg-slate-50/70 p-4 border-t border-slate-100 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    {/* Validés GOUV */}
                    <Link
                      href={`/hajj/admin/agences2/pelerins-gouv?id=${agence.id}`}
