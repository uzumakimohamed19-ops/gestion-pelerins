...
export default function PelerinProfilPage() {
  const searchParams = useSearchParams()
  const agenceId = searchParams.get('agenceId')
  const pelerinId = searchParams.get('pelerinId')
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-2 text-xs font-black uppercase text-gray-500 flex-wrap">
          ...
          <span>/</span>
          <Link href={`/hajj/admin/agences2/detail?id=${agenceId}`} className="hover:text-gray-900 transition-colors">
            {agence?.nom_agence || 'Agence'}
          </Link>
          <span>/</span>
          <Link href={`/hajj/admin/agences2/pelerins-gouv?id=${agenceId}`} className="hover:text-gray-900 transition-colors">
            Pèlerins GOUV
          </Link>
          <span>/</span>
