...
export default function AgencePelerinsPage() {
  const agenceId = useSearchParams().get('id')
  const db = usePowerSync()

  const [search, setSearch] = useState('')
  const [updatingId, setUpdatingId] = useState<string | null>(null)
  const { data: agenceRows, isLoading: agenceLoading } = useQuery<any>(
    'SELECT * FROM agences WHERE id = ?', [agenceId ?? ''],
  )
  const { data: pelerins, isLoading: pelerinsLoading } = useQuery<any>(
    'SELECT * FROM pelerins WHERE agence_id = ? ORDER BY created_at DESC', [agenceId ?? ''],
  )
  const agence = agenceRows?.[0] ?? null
  const loading = agenceLoading || pelerinsLoading
  ...
  const pelerinsFiltres = useMemo(() => (pelerins ?? []).filter((p: any) => {
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 space-y-6">
      ...
      <div className="bg-emerald-50 border border-emerald-200 p-4 rounded-2xl flex items-center justify-between">
        ...
        <Link
          href={`/hajj/admin/agences2/pelerins-gouv?id=${agenceId}`}
