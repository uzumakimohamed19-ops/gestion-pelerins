...
import {
  ...
  Building2, Phone, X, FileText, CheckSquare, Square, Filter,
...
export default function ListePelerins() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6 md:py-10">
          ...
          <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
            ...
            <div className="bg-gradient-to-r from-slate-900 to-blue-900 px-6 py-5">
              <div className="flex items-center gap-3 mb-1">
                ...
                <div>
                  <h2 className="text-white font-black text-lg">Export PDF</h2>
            ...
            <div className="p-6 space-y-4">
                ...
                <div className="space-y-2">
                  {[
                    ...
                    <button
                      ...
                      className={`w-full flex items-center gap-3 p-3.5 rounded-2xl border-2 transition-all duration-200 text-left ${opt.value ? 'border-blue-500 bg-blue-50' : 'border-gray-100 bg-gray-50 hover:border-gray-200'}`}
                    >
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 py-3 rounded-2xl border-2 border-gray-100 text-gray-700 font-black text-sm hover:bg-gray-50 transition-colors"
                >
                  Annuler
                </button>
                <button
                  ...
                  className="flex-1 py-3 rounded-2xl bg-gradient-to-r from-slate-900 to-blue-700 text-white font-black text-sm flex items-center justify-center gap-2 shadow-lg hover:shadow-xl transition-all hover:-translate-y-0.5 active:translate-y-0 disabled:opacity-60 disabled:cursor-not-allowed"
                >
                  {pdfGenerating ? (
      ...
      {/* ── HEADER ── */}
      <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-6 gap-5">
        <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
          {[
            ...
            <div key={i} className={`${stat.bg} border ${stat.border} rounded-2xl p-4`}>
              ...
              <p className="text-[10px] font-black uppercase text-gray-400 tracking-wider leading-tight">{stat.label}</p>
              <p className="text-[10px] font-bold text-gray-500 mt-0.5">{stat.sub}</p>
              {stat.bar !== null && (
      ...
      <div className="mb-4 relative group">
        <input
          ...
          className="w-full pl-12 pr-10 py-4 rounded-3xl border-2 border-gray-100 bg-white text-gray-900 font-bold focus:border-blue-600 focus:ring-0 outline-none transition-all duration-200 shadow-sm group-hover:border-gray-200 focus:shadow-md"
        />
        <Search className="absolute left-4 top-4 text-gray-400 group-focus-within:text-blue-600 transition-colors duration-200" size={22} />
        {searchTerm && (
          <button onClick={() => setSearchTerm('')} className="absolute right-4 top-4 text-gray-300 hover:text-gray-500 transition-colors">
            <X size={20} />
          </button>
        )}
      </div>
      ...
      <div className="mb-4 flex items-center gap-2 flex-wrap">
        <button
          ...
          className={`flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-black transition-all duration-200 active:scale-95 border ${showFilterPanel || hasActiveFilters ? 'bg-blue-600 text-white border-blue-600 shadow-md' : 'bg-gray-100 text-gray-600 border-transparent hover:bg-gray-200'}`}
        >
        ...
        {(['all', 'complet', 'partiel', 'non_paye'] as PaiementStatut[]).map(s => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-1 px-3 py-2 rounded-xl text-xs font-black transition-all duration-200 active:scale-95 ${statutPaiementFilter === s ? 'bg-gray-900 text-white shadow-sm' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
            >
              {labels[s]}
            </button>
          )
        ...
        {hasActiveFilters && (
          <button
            ...
            className="flex items-center gap-1 px-3 py-2 rounded-xl text-xs font-black bg-red-50 text-red-600 hover:bg-red-100 transition-colors border border-red-100 active:scale-95"
          >
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {[
                ...
                { key: 'gouv', label: '🏛 Plateforme Gouv uniquement' },
                { key: 'nusuk', label: '🕌 Nusuk uniquement' },
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all duration-150 active:scale-95 ${plateformeFilter === opt.key ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                >
                  {opt.label}
                </button>
              ))}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {([
                ...
                { type: 'agence', icon: <Building2 size={12} />, label: 'Agence / Associé' },
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-black transition-all duration-200 active:scale-95 ${activeFilterType === f.type ? 'bg-blue-600 text-white shadow-sm' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                >
                  {f.icon} {f.label}
                </button>
              ))}
      ...
      {/* ── SELECT ALL BAR ── */}
      {selectMode && (
        <div className="mb-4 bg-violet-50 border border-violet-100 rounded-2xl px-4 py-3 flex items-center justify-between animate-in fade-in slide-in-from-top-2 duration-200">
          <div className="flex items-center gap-3">
            <button onClick={toggleSelectAll} className="flex items-center gap-2 text-violet-700 font-black text-sm hover:text-violet-900 transition-colors">
          ...
          {selectedIds.size > 0 && (
            <button
              ...
              className="flex items-center gap-1.5 bg-violet-600 text-white px-4 py-2 rounded-xl text-xs font-black hover:bg-violet-700 transition-colors active:scale-95"
            >
              <FileText size={14} /> Export sélection ({selectedIds.size})
            </button>
          )}
        </div>
      )}
      ...
      <div className="space-y-4 md:space-y-0">
        {loading ? (
          ...
          <div className="py-20 flex flex-col items-center justify-center gap-4 text-center">
            ...
            <div>
              <p className="font-black text-gray-700 text-lg">Aucun résultat</p>
              <p className="text-gray-400 font-medium text-sm mt-1">Essayez de modifier vos filtres ou votre recherche</p>
            </div>
            <button onClick={clearAllFilters} className="text-blue-600 font-black text-sm hover:underline">Réinitialiser tout</button>
          </div>
        ) : (
          <>
            {/* ── MOBILE CARDS ── */}
                  ...
                  >
                    ...
                    <div className="px-4 pt-4 pb-3">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <h3 className="font-black text-gray-900 text-base leading-tight truncate">{p.prenom} {p.nom_complet}</h3>
                          <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                            <span className="font-mono text-[10px] bg-gray-100 px-2 py-0.5 rounded-lg text-gray-600 font-bold">{p.num_passeport}</span>
                            {p.telephone_pelerin && (
                              <span className="text-[10px] text-gray-400 font-medium">{p.telephone_pelerin}</span>
                            )}
                          </div>
                        </div>
                    ...
                    <div className="border-t border-gray-50 px-4 py-3 flex items-center justify-between gap-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex gap-2">
                          <button
                            ...
                            className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[9px] font-black transition-all active:scale-90 border ${p.sur_plateforme_gouv ? 'bg-green-50 border-green-200 text-green-700' : 'bg-gray-50 border-gray-200 text-gray-400'}`}
                          >
                          ...
                          <button
                            ...
                            className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[9px] font-black transition-all active:scale-90 border ${p.sur_plateforme_nusuk ? 'bg-blue-50 border-blue-200 text-blue-700' : 'bg-gray-50 border-gray-200 text-gray-400'}`}
                          >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Link
                          ...
                          className="bg-gray-900 text-white hover:bg-blue-600 px-4 py-2 rounded-xl text-xs font-black flex items-center gap-1 transition-all duration-200 active:scale-95 shrink-0"
                        >
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gray-50/50 border-b border-gray-100">
                    ...
                    <th className="px-6 py-5 text-[10px] font-black text-gray-400 uppercase tracking-widest">Plateformes</th>
                    <th className="px-6 py-5 text-[10px] font-black text-gray-400 uppercase tracking-widest">Dossier</th>
