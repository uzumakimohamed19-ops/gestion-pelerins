...
export default function NouvelleOperation() {
  ...
  benefice: beneficePrevu,
    ...
    try {
      const operation = { id: crypto.randomUUID(), created_at: new Date().toISOString(), ...payload }
      const columns = Object.keys(operation)
      const values = columns.map((column) => operation[column as keyof typeof operation] ?? null)
      await db.execute(
      ...
      alert("Erreur lors de l'enregistrement : " + (error?.message || 'Erreur locale'))
    }
    setLoading(false)
  }
