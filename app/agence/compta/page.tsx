...
function FormDepense({ agenceId, onAdded, onClose }: {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      await db.execute(
        'INSERT INTO depenses (id, agence_id, libelle, categorie, montant, mode_paiement, date_depense, notes, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [crypto.randomUUID(), agenceId, libelle.trim(), categorie, +montant, mode, date, notes.trim() || null, new Date().toISOString()],
      )
    } catch (error: any) {
      err = error instanceof Error ? error : new Error('Erreur locale')
    }
    setSaving(false)
...
export default function ComptabiliteAgence() {
  ...
  const operationFilters = ['agence_id = ?']
  ...
  const { data: localDepenses, isLoading: depensesLoading } = useQuery<Depense>(
    `SELECT * FROM depenses WHERE ${expenseFilters.join(' AND ')} ORDER BY date_depense DESC`,
    expenseParams,
  )
  const { data: localCorbeille, isLoading: corbeilleLoading } = useQuery<DepenseSupprimee>(
    'SELECT * FROM depenses_supprimees WHERE agence_id = ? ORDER BY supprime_le DESC LIMIT 200',
    [agenceId ?? ''],
  )

  useEffect(() => {
    const rows = operations ?? []
    const nextStats: Stats = { totalVentes: 0, totalAchats: 0, totalFraisAnnexes: 0, totalBenefice: 0, totalMontantVerse: 0, totalCreances: 0, count: rows.length, parType: {}, parModePaiement: {} }
    rows.forEach((operation: any) => {
      nextStats.totalVentes += Number(operation.prix_vente || 0)
      nextStats.totalAchats += Number(operation.prix_achat || 0)
      nextStats.totalFraisAnnexes += Number(operation.frais_annexes || 0)
      nextStats.totalBenefice += Number(operation.benefice || 0)
      nextStats.totalMontantVerse += Number(operation.montant_verse || 0)
      nextStats.totalCreances += Math.max(0, Number(operation.prix_vente || 0) - Number(operation.montant_verse || 0))
      const type = operation.type_activite || 'AUTRE'
      nextStats.parType[type] ??= { ventes: 0, benefice: 0, count: 0 }
      nextStats.parType[type].ventes += Number(operation.prix_vente || 0)
      nextStats.parType[type].benefice += Number(operation.benefice || 0)
      nextStats.parType[type].count += 1
      const mode = operation.mode_paiement || 'NON_DEFINI'
      nextStats.parModePaiement[mode] = (nextStats.parModePaiement[mode] || 0) + Number(operation.prix_vente || 0)
    })
    const nextDepenses = (localDepenses ?? []) as Depense[]
    setStats(nextStats)
    setDepenses(nextDepenses)
    setTotalDep(nextDepenses.reduce((total, expense) => total + Number(expense.montant || 0), 0))
    setCorbeille((localCorbeille ?? []) as DepenseSupprimee[])
    setLoading(operationsLoading)
    setLoadDep(depensesLoading || corbeilleLoading)
  }, [operations, localDepenses, localCorbeille, operationsLoading, depensesLoading, corbeilleLoading])

  const refreshAllData = useCallback(() => undefined, [])

  const delDep = async (dep: Depense) => {
    if(!confirm(`Supprimer "${dep.libelle}" ?\nElle sera conservée dans la corbeille.`)) return
    await db.execute(
      'INSERT INTO depenses_supprimees (id, agence_id, depense_id, libelle, categorie, montant, mode_paiement, date_depense, notes, created_at, supprime_le) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
